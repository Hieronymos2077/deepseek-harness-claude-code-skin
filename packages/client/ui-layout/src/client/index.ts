  ...
  ctx.effect(() => {
    ...
    const disposeRegistration = ctx.slots.register({
      ...
      inject: (actions: PanelActions) => {
        ...
        return {
          selectSession: (id: SessionId) => { ctx.sessions.open(id) },
          // The canvas's New chat rides the SAME shared action as the
          // sidebar's New Session button, so the workspace resolution and the
          // default agent preset stay one implementation.
