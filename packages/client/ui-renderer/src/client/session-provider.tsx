 ...
 * boundary.
 */
export function SessionProvider({ empty, sessionId, children }: SessionProviderProps) {
  const host = useHost()
  ...
  useEffect(() => {
    if (sessionId !== undefined && id !== undefined) host.sessions.openView?.(sessionId)
  }, [host, id, sessionId])
  if (info === undefined || id === undefined) return <>{empty?.() ?? null}</>
  return (
