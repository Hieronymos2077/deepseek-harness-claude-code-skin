...
function openRouterPayloadHook(
  profile: ResolvedPiAiProviderProfile,
  model: Model<Api>,
): SimpleStreamOptions['onPayload'] | undefined {
  const routing = model.provider === 'openrouter' ? profile.openRouterProviderRouting?.[model.id] : undefined
  if (routing === undefined) return undefined
  return (payload) => {
    if (payload === null || typeof payload !== 'object' || Array.isArray(payload)) {
      throw new Error('OpenRouter payload is not an object')
    }
    return {
...
export class PiAiAdapter extends LlmAdapter {
  ...
  private async * streamWithSnapshot(
    ...
    try {
      ...
      const payloadHook = openRouterPayloadHook(profile, model)
      const events = snapshot.models.streamSimple(model, context, {
        ...
        ...payloadHook === undefined ? {} : { onPayload: payloadHook },
      })
