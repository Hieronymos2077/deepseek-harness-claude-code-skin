...
export interface PiAiProviderProfile {
  ...
  /** Per model OpenRouter endpoint constraints, applied only on the OpenRouter route. */
  openRouterProviderRouting?: Record<string, OpenRouterProviderRouting>
...
const openRouterProviderRouting: z<OpenRouterProviderRouting> = z.object({
...
const profile = z.object({
  ...
  openRouterProviderRouting: z.dict(openRouterProviderRouting),
...
export function resolveProfiles(
  ...
  for (const [provider, source] of entries) {
    // Emptiness is the test, not presence: schemastery materializes an absent
    // dict as `{}` (the same hazard `reasoningEfforts` above is a union for),
    // so a `!== undefined` guard here fired for EVERY route the moment the
    // section carried more than the openrouter one, and the whole namespace
    // fell back to the dormant empty route set with no diagnostic. Only a
    // routing pin somebody actually typed is refused off the openrouter route.
    if (source.openRouterProviderRouting !== undefined
      && Object.keys(source.openRouterProviderRouting).length > 0
      && provider !== 'openrouter') {
      throw new Error(`llm-pi-ai: provider "${provider}" sets openRouterProviderRouting, which only applies to the openrouter route`)
    }
